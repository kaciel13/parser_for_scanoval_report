...
# Словарь для хранения Lock для каждого CWE ID
cwe_locks = {}
            ...
            for bdu_id in bdu_ids:
                ...
                if status == 200 and html:
                    ...
                    if json_data_element:
                        ...
                        cwe_matches = re.findall(r'"cwe_id":"(\d+)"', json_text)
                        if cwe_matches:
                            # Извлекаем уникальные CWE ID
                            unique_cwe_ids = set(f"CWE-{cwe_id}" for cwe_id in cwe_matches)
                            for cwe_id in unique_cwe_ids:
                                link_cwe = f"https://cwe.mitre.org/data/definitions/{cwe_id.split('-')[1]}.html"

                                # Получаем или создаем Lock для данного CWE ID
                                if cwe_id not in cwe_locks:
                                    cwe_locks[cwe_id] = asyncio.Lock()

                                async with cwe_locks[cwe_id]:  # Используем Lock для синхронизации доступа к кэшу
                                    # Проверяем, есть ли данные в кэше для данного CWE
                                    attempts = 0
                                    while attempts < 3:  # Максимум 3 попытки
                                        if cwe_id in cwe_cache:
                                            likelihoods = cwe_cache[cwe_id]
                                            break  # Данные найдены в кэше, выходим из цикла
                                        else:
                                            print(f"Данные для {cwe_id} не найдены в кэше. Повторная попытка через 2 секунды...")
                                            await asyncio.sleep(2)  # Ждем 2 секунды перед повторной попыткой
                                            attempts += 1

                                    if attempts == 3:  # Если данные все еще не найдены после 3 попыток
                                    ...
                                    # Добавляем данные в список для каждого CWE ID
                                    data.append({
                                        'BDU ID': bdu_id,
                    ...
                    print(f"Не удалось получить данные для BDU ID: {bdu_id}")
...
async def main():
    ...
    # Перевод описаний CAPEC перед записью в файл
    print("Перевод описаний CAPEC на русский язык...")
    await translate_capec_descriptions(capec_data)
